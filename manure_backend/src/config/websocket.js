    ...
    try {
      ...
      if (sender === "esp32") {
        ...
        if (esp32Client && esp32Client.readyState === esp32Client.OPEN) {
          if(data.mode=="manual"){
            console.log("manual samma aai pugyo")
            if(data.device=="pump" && data.action=="on"){
              esp32Client.send("w");
              console.log("sending w")
            }
            if(data.device=="pump" && data.action=="off"){
              esp32Client.send("q");
              console.log("sending manual q")

            }if(data.device=="fan" && data.action=="on"){
              esp32Client.send("r");
              console.log("senfing manual r")

            }if(data.device=="fan" && data.action=="off"){
              esp32Client.send("s");
              console.log("sending manual s")
            }
